...
public func _createPixelBuffer(width: Int, height: Int, pixelFormat: OSType) -> CVPixelBuffer? {
    ...
    let releaseCallback: CVPixelBufferReleaseBytesCallback = { _, ptr in
        ...
        var pixelBuffer: CVPixelBuffer?
        let status = CVPixelBufferCreateWithBytes(nil, width, height, pixelFormat, data, bytesPerRow, releaseCallback, nil, nil, &pixelBuffer)
        if status != kCVReturnSuccess {
...
public extension CVPixelBuffer {
    ...
    */
    func deepCopy(withAttributes attributes: [String: Any] = [:]) -> CVPixelBuffer? {
        ...
        guard
            kCVReturnSuccess == CVPixelBufferLockBaseAddress(srcPixelBuffer, srcFlags)
        else {
            return nil
        }
        defer {
        ...
        guard
            status == kCVReturnSuccess, let dstPixelBuffer = maybePixelBuffer
        else {
            return nil
        }
        ...
        guard
            kCVReturnSuccess == CVPixelBufferLockBaseAddress(dstPixelBuffer, dstFlags)
        else {
            return nil
        }
        defer {
        ...
        for plane in 0...max(0, CVPixelBufferGetPlaneCount(srcPixelBuffer) - 1) {
            if let srcAddr = CVPixelBufferGetBaseAddressOfPlane(srcPixelBuffer, plane), let dstAddr = CVPixelBufferGetBaseAddressOfPlane(dstPixelBuffer, plane) {
